              ...
              <?php }}?>               
                     <button class="btn btn-sm" onclick="window.print()"> <i class="fa fa-print"></i> PRINT </button>
                      ...
                      <?php foreach ($sms as $row):?>
											<tr>
												<td>
													<span class="center">
                           							 <input type="checkbox" class="cc" name="sms_id[]" value="<?php echo $row['sms_id'];?>" />
                          							</span>                          </td>
                                    			<td><span class="label label-success"><?php echo $row['sending_number'];?></span></td>
                                    			<td><span class="label label-primary"><?php echo $row['receiver_number'];?></span></td>
                  ...
                  var sms_id = [];
                   jQuery('.cc:checked').each(function(i,e){
                     sms_id.push(jQuery(this).val());
                  });
                  if(sms_id ==''){
                    alert('PLEASE SELECT ATLEAST ONE SMS TO DELETE!');
                  }else{
